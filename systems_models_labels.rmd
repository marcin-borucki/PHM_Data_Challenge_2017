...
test_data$Speed2 <- test_data$Speed**(2)
...
failed <- read.csv("azs_suspect2.csv")

merge(test_azs,failed,by="ExperimentID")
...
test_data_summ %>% 
  filter(
       th_b1r_1_low - th_b1r_1_sd*2 > b1r_1_res_median | b1r_1_res_median > th_b1r_1_high + th_b1r_1_sd*2 |
          th_b1r_4_low - th_b1r_4_sd*2 > b1r_4_res_median | b1r_4_res_median > th_b1r_4_high + th_b1r_4_sd*2 |
          th_b1r_3_low - th_b1r_3_sd*2 > b1r_3_res_median | b1r_3_res_median > th_b1r_3_high + th_b1r_3_sd*2 |
          th_b1r_2_low - th_b1r_2_sd*2 > b1r_2_res_median | b1r_2_res_median > th_b1r_2_high + th_b1r_2_sd*2 |
         th_b2r_1_low - th_b2r_1_sd*2 > b2r_1_res_median | b2r_1_res_median > th_b2r_1_high + th_b2r_1_sd*2 |
         th_b2r_3_low - th_b2r_3_sd*2 > b2r_3_res_median | b2r_3_res_median > th_b2r_3_high + th_b2r_3_sd*2 |
         th_b2r_2_low - th_b2r_2_sd*2 > b2r_2_res_median | b2r_2_res_median > th_b2r_2_high + th_b2r_2_sd*2 |
         th_b2r_4_low - th_b2r_4_sd*2 > b2r_4_res_median | b2r_4_res_median > th_b2r_4_high + th_b2r_4_sd*2
         ) %>% select(ExperimentID) %>%
   count()

# front bogie lags

b1_flags <- read.csv("b1_flags2.csv")

#b1z flag
b1z_labels <-test_data_summ %>% 
  mutate(
    b1z_flag = ifelse(b1z_3_res_median > th_b1z_3_high + th_b1z_3_sd,"hi","healthy"),
    b1z_flag = ifelse(th_b1z_3_low - th_b1z_3_sd > b1z_3_res_median ,"low",b1z_flag),
    b1r_flag = ifelse(b1r_3_res_median > th_b1r_3_high + th_b1r_3_sd,"hi","healthy"),
    b1r_flag = ifelse(th_b1r_3_low - th_b1r_3_sd > b1r_3_res_median ,"low",b1r_flag),
    b1p_flag = ifelse(b1p_3_res_median > th_b1p_3_high + th_b1p_3_sd,"hi","healthy"),
    b1p_flag = ifelse(th_b1p_3_low - th_b1p_3_sd > b1p_3_res_median ,"low",b1r_flag)
  ) %>% select(ExperimentID, b1z_flag, b1r_flag, b1p_flag)

b1z_labels <- merge(b1z_labels,b1_flags,by=c("b1z_flag","b1r_flag","b1p_flag"),all.x=TRUE)


b2_flags <- read.csv("b2_flags2.csv")

#b1z flag
b2z_labels <-test_data_summ %>% 
  mutate(
    b2z_flag = ifelse(b2z_3_res_median > th_b2z_3_high + th_b2z_3_sd,"hi","healthy"),
    b2z_flag = ifelse(th_b2z_3_low - th_b2z_3_sd > b2z_3_res_median ,"low",b2z_flag),
    b2r_flag = ifelse(b2r_3_res_median > th_b2r_3_high + th_b2r_3_sd,"hi","healthy"),
    b2r_flag = ifelse(th_b2r_3_low - th_b2r_3_sd > b2r_3_res_median ,"low",b2r_flag),
    b2p_flag = ifelse(b2p_3_res_median > th_b2p_3_high + th_b2p_3_sd,"hi","healthy"),
    b2p_flag = ifelse(th_b2p_3_low - th_b2p_3_sd > b2p_3_res_median ,"low",b2r_flag)
  ) %>% select(ExperimentID, b2z_flag, b2r_flag, b2p_flag)

b2z_labels <- merge(b2z_labels,b2_flags,by=c("b2z_flag","b2r_flag","b2p_flag"),all.x=TRUE)

b_labels <- merge(b1z_labels, b2z_labels, by="ExperimentID")

b_labels <- b_labels %>%
  mutate(var.x = ifelse(is.na(var.x),"",as.character(var.x)),
         var.y = ifelse(is.na(var.y),"",as.character(var.y)),
         state = ifelse(var.y=="" & var.x !="",var.x,"healthy"),
         state = ifelse(var.y!="" & var.x =="",var.y,state),
         state = ifelse(var.y!="" & var.x !="",paste(var.x,var.y,sep="+"),state)
         ) %>% select(ExperimentID,state)
write.csv(b_labels, file="bogie_labels_20_08_2017.csv")
summary(b_labels)
```
