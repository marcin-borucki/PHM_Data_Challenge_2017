...
```{r TrackModel1, eval=FALSE, include=FALSE}
...
# EID = 168 # failed spring

p <- gains_test_all_sec  %>% filter(ExperimentID == EID)  %>% 
    slice(grep("^azps_1", p_l, invert = FALSE)) %>%
    ggplot(., aes(x = lp_off)) +
      # geom_line(aes(y =  gain.x, color =  paste("fit_var",p_l)))  +
      # geom_line(aes(y =  gain.y, color =  paste("real_val",p_l)))  +
    geom_line(aes(y =  residuals_gain, color =  paste("res_val",p_l)))
show(p)


# EID = 1 # rather ok
# Seondary susp review
p <- gains_test_all_sec  %>% filter(ExperimentID == EID)  %>% 
    slice(grep("^azps_1", p_l, invert = FALSE)) %>%
    ggplot(., aes(x = lp_off)) +
      # geom_line(aes(y =  gain.x, color =  paste("fit_var",p_l)))  +
...
boxplot(sec_res_summ$q75,  boxwex=0.1)


#  Secondary susp sides

ps <- gains_test_sum_sec  %>% #  filter(q75 > 0.05 ) %>%
    ggplot(., aes(x = p_l,
                  y = q75
...
outlier_values <- boxplot(q75 ~ part, data = gains_test_sum_sec)$out  # outlier values.
boxplot(sec_res_summ$q75, boxwex=0.1)
